...
s.addText([
  { text: "26.5% observed churn = ", options: { color: ICE } },
  { text: "$1.67M annualized exposure", options: { color: WHITE, bold: true } },
  { text: ". Here's how to target it.", options: { color: ICE } },
...
s.addText("annualized revenue exposure", { x: 9.2, y: 4.05, w: 3.5, h: 0.5,
  fontFace: BODY_F, fontSize: 13, color: ICE, align: "center", margin: 0 });
notes[1] = "In this dataset, 26.5% of customers churned, representing about $1.67M in annualized recurring-revenue exposure. This readout shows who leaves, why, and the ROI of testing a targeted response.";

// ================= Slide 2 - The problem =================
s = p.addSlide();
title(s, "The problem: expensive, and concentrated");
statCard(s, 0.7, 2.2, 3.8, 2.4, "26.5%", RED, "of customers in the dataset churned");
statCard(s, 4.77, 2.2, 3.8, 2.4, "$1.67M", RED, "annualized recurring-revenue exposure");
statCard(s, 8.84, 2.2, 3.8, 2.4, "$74 vs $61", BLUE, "churned pay more than retained (monthly)");
s.addText("Churn isn't just a rate - it's revenue, and the customers leaving are the higher-value ones. That makes a targeted retention test worth evaluating.",
  { x: 0.7, y: 5.1, w: 11.9, h: 0.9, fontFace: BODY_F, fontSize: 17, color: INK, margin: 0 });
notes[2] = "Churners pay $74/mo vs $61 for those who stay - so this is expensive churn, which is exactly what makes a paid retention play worth testing.";
...
s.addText("A logistic-regression model scores every customer, so retention budget can be aimed at the people most likely to leave - not sprayed across the base.",
  { x: 7.8, y: 4.35, w: 4.85, h: 1.3, fontFace: BODY_F, fontSize: 15, color: INK, margin: 0, valign: "top" });
notes[5] = "The model (AUC 0.85) concentrates two-thirds of observed churn into the top 30% of customers by risk - the basis for efficient targeting.";
...
bullets(s, 7.6, 2.0, 5.3, [
  ...
  { h: "$347K annualized net benefit", d: "on $127K campaign spend", c: GREEN },
  { h: "2.7x ROI", d: "still positive at a 15% save rate", c: GREEN },
]);
notes[6] = "Base case: $60 offer to the top 30% risk, 30% save rate -> $347K annualized net benefit, 2.7x ROI. Sensitivity stays positive down to a 15% save rate.";
...
s.addText("annualized net", { x: 9.95, y: 3.1, w: 2.65, h: 0.4, fontFace: BODY_F, fontSize: 14,
...
notes[7] = "Four moves, one number: ~$347K annualized net benefit. Validate with an A/B holdout before full rollout.";
